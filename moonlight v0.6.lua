local scriptVersion = "0.6" -- do not edit this unless you know what you're doing
...
local displayTogglesStyles = gui.Combobox(mainTabInterface, "indicatorStyle", "Indicator Style", unpack({"New", "Old"}))
displayTogglesStyles:SetDescription("Choose an indicator style.")
displayTogglesStyles:SetInvisible(true)
...
if math.random(1, 10) == 8 then
	additionalText = [[
If you like what you see, how about giving me
a +rep on my profile? Thanks!
...
callbacks.Register("Draw", function()
    if scriptToggle:GetValue() and espTabToggle:GetValue() then
        gui.SetValue("esp.overlay.friendly.precision", enhanceEspPrecision:GetValue())
        gui.SetValue("esp.overlay.enemy.precision", enhanceEspPrecision:GetValue())
    end
end)
local indicatorListSliderX = gui.Slider(mainScriptToggleBox, "indicatorSavePositionX", "thisisatest1", moduleWidth*0.70, 0, screenWidth)
local indicatorListSliderY = gui.Slider(mainScriptToggleBox, "indicatorSavePositionY", "thisisatest2", moduleHeight+14, 0, screenHeight)
...
callbacks.Register("Draw", function()
	if scriptToggle:GetValue() and displayToggles:GetValue() then
		displayTogglesStyles:SetInvisible(false)
		if displayTogglesStyles:GetValue() == 0 then
			displayTogglesBox:SetInvisible(true)
			if entities.GetLocalPlayer() ~= nil then
				draw.SetFont(moonlightFont3)
				draw.Color(0, 0, 20, 100)
				draw.FilledRect(indicatorListX, indicatorListY-5, indicatorListX+70, indicatorListY + #indicatorItems.toggleList*14+5)
				draw.ShadowRect(indicatorListX, indicatorListY-5, indicatorListX+70, indicatorListY + #indicatorItems.toggleList*14+5, 2)
				draw.Color(57, 108, 255, 255)
				draw.Line(indicatorListX, indicatorListY-6, indicatorListX+70, indicatorListY-6)
				...
				for i=1, #indicatorItems.toggleList, 1 do
					...
					draw.TextShadow(indicatorListX+5, indicatorListY+i*14-10, indicatorItems.toggleList[#indicatorItems.toggleList+1-i])
				end
			end
		elseif displayTogglesStyles:GetValue() == 1 then
...
updatePlayerAvatars:SetDescription("Download all avatars. WILL FREEZE YOUR GAME!")
callbacks.Register("Draw", function()
	if playerAvatarUpdateModeItems[1]:GetValue() then
		updatePlayerAvatars:SetInvisible(false)
	else
		updatePlayerAvatars:SetInvisible(true)
	end
end)

local specListSliderX = gui.Slider(guiBoxOtherInformation, "specListSavePositionX", "thisisatest1", 25, 0, screenWidth)
local specListSliderY = gui.Slider(guiBoxOtherInformation, "specListSavePositionY", "thisisatest2", 660, 0, screenHeight)
     ...
     if entities.GetLocalPlayer() ~= nil then
      local players = entities.FindByClass("CCSPlayer")
        for i=1, #players do
            ...
            if player ~= entities.GetLocalPlayer() and player:GetHealth() <= 0 then
...
local bombInformation = gui.Checkbox(guiBoxOtherInformation, "toggleBombInformation", "Bomb Information", false)
bombInformation:SetDescription("Enable moonlight's bomb information. Coming Soon.")
bombInformation:SetDisabled(true)

local guiBoxOtherImportant = gui.Groupbox(otherTab, "Important Information", 305, 238, 285, 50)
callbacks.Register("Draw", function()
	if playerAvatarUpdateModeItems[1]:GetValue() then
		guiBoxOtherImportant:SetPosY(295)
	else
		guiBoxOtherImportant:SetPosY(238)
	end
end)
local otherImportantNotice = gui.Text(guiBoxOtherImportant, [[
Regarding "Avatar Update Mode":

- it will freeze your game for 1-10 seconds
depending on how many new players are on the
server each time it updates avatars.

- while I don't recommend updating avatars
on player connect or disconnect (that can 
cause unwanted game lag during important
moments) you can still use it if you want to.

- as always, there is most likely a better
way of implementing this, but I'm too stupid
to know it and this works just fine imho.

- if you have any questions, please post them
on the thread in the forums - that way if
someone else has the same problem there will be
no need to repeat the same explanation 10
times over.


This lua is still under heavy development.
